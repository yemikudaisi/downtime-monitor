...
pub fn delete_tables() -> rusqlite::Result<()> {
    let mut conn = get_connection().unwrap();
    let tx = conn.transaction().unwrap();
    tx.execute("DROP TABLE IF EXISTS heartbeats", []).expect("Drop heartbeat table failed");
    tx.execute("DROP TABLE IF EXISTS services", []).expect("Drop heartbeat table failed");
    tx.commit()
}
...
pub mod heartbeat {
    ...
    use super::*;
    use crate::types::{Heartbeat, ServiceStatus};
...
pub mod service{
    use super::*;
    use crate::types::{ServiceParameters, ServiceType};
    ...
    pub fn get_by_id(id: i64) -> rusqlite::Result<ServiceParameters> {
        ...
        let row = conn.query_row(query, params![id], |row| {
            ...
            Ok(ServiceParameters {
                ...
                service_type: ServiceType::from_str(&service_type_str),
            })
        });
...
mod tests {
    ...
    use crate::{types::{Heartbeat, ServiceParameters, ServiceStatus}, db::{create_tables, delete_tables, get_connection, get_table_count, reset_tables}};
